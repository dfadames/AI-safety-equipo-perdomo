...
\begin{itemize}
    ...
    \item \textbf{Data:} \texttt{resultados\_v1/} (the 37 episodes of this paper, task version v1): one log per episode (\texttt{.jsonl}) with its configuration, per-episode and per-point CSVs, the judge's scores (\texttt{monitor.csv}, \texttt{monitor\_sin\_regla.csv}) and an interactive map of every episode (\texttt{mapa.html}). Figures, tables and the numbers of the first draft are regenerated with \texttt{python -m hormiguero.figuras --resultados resultados\_v1 --tarea v1}. The analyses added in this revision (identity-level oracle check, cut decomposition, association statistics, per-command behavioural rates) are regenerated with \texttt{paper/reanalisis\_revision.py}.
