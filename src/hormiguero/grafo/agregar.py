...
def curvas(filas: list[dict]) -> list[dict]:
    """Una fila por punto (condición, N). Es lo que se grafica."""
    grupos = defaultdict(list)
    for f in filas:
    ...
    for (cond, n), g in sorted(grupos.items()):
        ...
        out.append({
            "condicion": cond, "N": n, "episodios": len(g),
...
def main(argv=None):
    ...
    if csv_out:
        ...
        if bloqueos:
            ...
            print(f"  lista_de_bloqueo.json - {len(bloqueos)} episodios para repetir bloqueados")
    print()
    return filas
