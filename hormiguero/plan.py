...
Por que un solo script y no `runner uno` en un bucle:

  - El cluster de Docker se levanta UNA vez por cada tamano de poblacion, no
    por episodio: levantar y bajar en cada uno son minutos por corrida.
  - Cubre el diseno completo, controles incluidos (`benigna`, `honestidad`).
  - Es REANUDABLE: cuenta lo que ya hay en results/ y corre solo lo que
...
def hechos(raiz: str, version_tarea: str = VERSION_TAREA,
    ...
    for cfg in base.rglob("*.cfg.json"):
        ...
        # ensayo dejado en results/ le quitaba el lugar a un episodio real.
        if d.get("proveedor") == "simulado":
            continue
        # Ni un episodio de OTRA version de la tarea: en v1 validar aceptaba
        # cualquier texto que contuviera las partes, asi que esos episodios no
        # llenan ningun punto del lote nuevo. Los cfg viejos no traen el campo
        # y son v1.
...
def main(argv=None):
    ...
    ap.add_argument("--logs", default=None, help="por defecto, results/")
    ...
    if a.simulado and not a.logs:
        # El ensayo corre sobre una COPIA de results/. Si escribiera en la
        ...
        raiz = str(copia)
        print(f"  Ensayo sobre una copia, no sobre results/:\n    {raiz}\n")

    sin_docker = not con_docker

    # Docker se levanta por TAMANO de poblacion, no por episodio: con N=8 hacen
    # falta 8 cajas y con N=4 cuatro. Levantar y bajar en cada episodio son
    # minutos por corrida.
    por_n = {}
