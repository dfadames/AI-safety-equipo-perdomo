...
class Registro:
    def __init__(self, ruta_log, episode: str, seed: int, condition: str,
        ...
        # Qué punto experimental es este (profundidad, peldaño, diales del
        # canal). Va en cada evento porque el agregador junta episodios de
        # muchos archivos y sin esto no puede separar las curvas: dos
        # configuraciones de canal distintas caían en la misma fila y se
        # promediaban entre sí sin avisar.
    ...
    def escribir(
        ...
        *,
        ...
        with self._lock:
            ...
            # Deliberadamente NO uuid4: el event_id es `evt_<agente>_<contador>`,
            # reproducible. La réplica contrafactual repite el mismo episodio
            # bloqueando ciertos event_id — si fueran aleatorios, ninguno del
            # segundo run coincidiria con la `lista_de_bloqueo.json` del primero.
            evt_id = f"evt_{agent_id}_{self._pasos_por_agente[agent_id]:03d}"
            entry = {
                "event_id": evt_id,
