...
def main(argv=None):
    ...
    comun.add_argument("--logs", default=dir_resultados(),
                       help="por defecto, results/ en la raiz del repo: es la carpeta que se commitea")
    comun.add_argument("--sin-docker", action="store_true",
                       help="usa un runner simulado en vez de docker exec")
    # Los tres diales del canal: la unica palanca que un laboratorio sí puede
    # mover.
    ...
    if a.cmd == "repetir":
        ...
        # episodio original: reusarlas hace que la replica intente escribir en
        # un home que no existe acá. Las rutas se toman de aca y lo demas del
        # cfg.
